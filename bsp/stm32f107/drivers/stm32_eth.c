...
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct)
{
  ...
  //allen: forbidden port6 in 100M mode
...
extern unsigned char ReadButton();
...
unsigned char g_rxdump=0;
unsigned char g_txdump=0;
...
static rt_err_t rt_stm32_eth_init(rt_device_t dev)
{
  ...
  if(Value)
  {
	rt_kprintf("Switch link ok \r\n");
  }
  else
  {
	rt_kprintf("***********link not ok, need to reconfig********\r\n");
  }
...
rt_err_t rt_stm32_eth_tx( rt_device_t dev, struct pbuf* p)
{
    ...
    /* get free tx buffer */
	{
        ...
        if (result != RT_EOK) 
        {
			STM32_ETH_TRACE("tx take tx buff sem failed\r\n");
			return -RT_ERROR;
        }
    }
...
rt_err_t rt_stm32_eth_tx( rt_device_t dev, struct pbuf* p)
{
	...
	//memcpy((__IO uint8_t *)DMATxDescToSet->Buffer1Addr,(char *)p->payload,p->len);
	ptr = (char *)p->payload;
	for(i=0;i<p->len;i++)
	{
		 (*(__IO uint8_t *)((DMATxDescToSet->Buffer1Addr) + i)) = *(ptr+i);;
	}
    ...
    /* Setting the Frame Length: bits[12:0] */
    DMATxDescToSet->ControlBufferSize = (p->len & ETH_DMATxDesc_TBS1);
    /* Setting the last segment and first segment bits (in this case a frame is transmitted in one descriptor) */
    DMATxDescToSet->Status |= ETH_DMATxDesc_LS | ETH_DMATxDesc_FS;
...
struct pbuf *rt_stm32_eth_rx(rt_device_t dev)
{
   ...
   unsigned int i;
	...
	p = pbuf_alloc(PBUF_LINK, framelength, PBUF_RAM);
        if (p != RT_NULL)
        {
			//memcpy(p->payload, (void *)DMARxDescToGet->Buffer1Addr, framelength);
			ptr = (unsigned char*)p->payload;
			for(i=0;i<framelength;i++)
				*(ptr+i) = (*(__IO uint8_t *)((DMARxDescToGet->Buffer1Addr) + i));
#ifdef ETH_RX_DUMP
	if(g_rxdump)
 ...
 */
u16 Show_ETH_PHY(u16 PHYAddress, u16 PHYRegAddr)
{
  u16 PHYRegData;
  PHYRegData = ETH_ReadPHYRegister(PHYAddress, PHYRegAddr);
  rt_kprintf("\n\r ETH_ReadPHYRegister(%d, %d):0x%X", PHYAddress, PHYRegAddr, PHYRegData);
  return PHYRegData;
}

void Check_ETH_PHY(u16 PHYAddress)
{
    Show_ETH_PHY(PHYAddress, 0);
    Show_ETH_PHY(PHYAddress, 1);
    Show_ETH_PHY(PHYAddress, 2);
    Show_ETH_PHY(PHYAddress, 3);
    Show_ETH_PHY(PHYAddress, 4);
    Show_ETH_PHY(PHYAddress, 5);
}
...
#if 1
  /* Configure PD8 as input -- workaround, by Henry Song */
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
  GPIO_Init(GPIOD, &GPIO_InitStructure);  
//#else
   //allen:1207, enable MCO output
  ...
  //Reset Signal PC12 to ETH Switch
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
  GPIO_Init(GPIOC, &GPIO_InitStructure);

  // Reset Signal PA6 to DSL
...
void rt_hw_stm32_eth_init()
{
    ...
    stm32_eth_device.dev_addr[2] = mymac[2];//0x44;
    // generate MAC addr from 96bit unique ID (only for test)
    stm32_eth_device.dev_addr[3] = mymac[3];//0x55;//*(rt_uint8_t*)(0x1FFFF7E8+7);
    stm32_eth_device.dev_addr[4] = mymac[4];//0x66;//*(rt_uint8_t*)(0x1FFFF7E8+8);
    stm32_eth_device.dev_addr[5] = mymac[5];//0x77;//*(rt_uint8_t*)(0x1FFFF7E8+9);
...
static void set_txdump(unsigned char value)
{
    g_txdump=value;
}
FINSH_FUNCTION_EXPORT(set_txdump, enable tx);

static void set_rxdump(unsigned char value)
{
    g_rxdump=value;
}
FINSH_FUNCTION_EXPORT(set_rxdump, enable rx);
